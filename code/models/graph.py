...
class GraphMultiHeadAttention(nn.Module):
    def __init__(self,edges,input_size,hidden_size,graph_drop=0.0,nhead=4):
        super(GraphMultiHeadAttention, self).__init__()
        assert hidden_size%nhead == 0
        ho = int(hidden_size/nhead)
        self.nhead = int(nhead)
        self.head_graph = nn.ModuleList([GraphAttentionLayer(edges,input_size,ho,graph_drop) for _ in range(self.nhead)])
...
class GraphAttentionLayer(nn.Module):
    def __init__(self,edges,input_size,hidden_size,graph_drop):
        ...
        nn.init.xavier_uniform_(self.W, gain=nn.init.calculate_gain('relu'))
        self.edges = edges
        self.W_edge = nn.ModuleList([nn.Linear(2*hidden_size,1,bias=True) for i in (self.edges)])
        for m in self.W_edge:
            nn.init.xavier_uniform_(m.weight, gain=nn.init.calculate_gain('relu'))
    ...
    def forward(self, nodes_embed,node_adj):
        ...
        out = self.drop(torch.relu(torch.matmul(attention, h)))
